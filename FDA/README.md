# Akkuro Lending Data Analytics

Akkuro Lending Data Analytics (ALDA) is the application which exposes the data of the different Akkuro Lending components in a standard way (for all customers in the same way). If a customer only uses one component, the complete data model will be used, but only a subset will be filled. 

## Process based data model

Akkuro Lending is a process based application. This means that all changes are implemented using a pre-configured process/workflow. All data in ALDAs data model is related to a process number. This means the following:
...
## Visual overview 'Data export StandardZone'
[Graphical overview (latest released version 2026.1)](v2026.1/relationships.html) of the Akkuro Lending Data Analytics StandardZone data model generated by Schemaspy. 
